...
A read-only markdown tab kind: sanitized rendering that can never script the app, configurable browsable roots (with shell-trust direct opens), in-pane relative-link navigation, and a lazily loaded renderer with refresh.
...
### Requirement: Markdown rendering matches the apex preview component

The markdown viewer's rendering SHALL be overhauled to match the quality of the apex project's
markdown/preview component — GFM support, syntax-highlighted fenced code blocks, and theme-aware
typography and spacing (headings, lists, tables, blockquotes, inline code) derived from palmux's
theme tokens. The look (rendering, styles adapted to palmux, spacing) SHALL follow apex's component as
the reference. The existing sanitization guarantee (rendered markdown can never script the app) SHALL
be preserved regardless of the rendering library used.

#### Scenario: GFM and code highlighting

- **WHEN** a markdown file with GFM tables and a fenced code block is opened
- **THEN** the table renders styled and the code block is syntax-highlighted, matching apex's presentation

#### Scenario: Theme-aware, well-spaced typography

- **WHEN** markdown is rendered under any palmux theme
- **THEN** headings/lists/blockquotes/inline code use theme-token colors with apex-equivalent spacing, not the current bare styling

#### Scenario: Sanitization preserved

- **WHEN** hostile markdown (script/SVG/event-handler payloads) is rendered
- **THEN** no script executes — the sanitization boundary holds under the new renderer
