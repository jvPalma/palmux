...
The repository enforces code quality through freshly-introduced Oxlint + Oxfmt tooling — single reproducible `yarn lint` / `yarn format` commands with checked-in configs — and requires that every catalogued quality refactor be behavior-preserving and gated green by typecheck + the existing test suites.
