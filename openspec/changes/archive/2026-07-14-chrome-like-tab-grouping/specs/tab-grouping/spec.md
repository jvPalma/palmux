...
On desktop, each group SHALL render a colored chip (its name, or a compact colored dot when unnamed) immediately before its first member, with member tabs visually framed as one cluster in the group color; a member tab SHALL ALSO keep its own name and color accent (the group color/name and the per-tab color/name are independent, both shown). The chip SHALL offer, via its context menu, rename, recolor, ungroup (members stay as plain tabs), and close-all. A tab SHALL be groupable via its own context menu ("New group from this tab", and "Add to <group>" for each live group).

#### Scenario: Chip, cluster, and per-tab color coexist

- **WHEN** a group "work" (blue) holds a tab that is itself colored red
- **THEN** a blue "work" chip precedes the cluster, the tab carries the blue cluster framing, AND the tab keeps its own red accent — both colors visible
...
- **WHEN** the chip menu's Ungroup is used
- **THEN** the members remain as ordinary tabs in place (keeping their own colors) and the chip disappears

#### Scenario: Close-all is kind-aware

- **WHEN** the chip menu's Close all is used on a group containing a dirty editor tab
- **THEN** the confirmation warns about the unsaved editor (never bulk-kills it silently); on accept, every member is closed
...
Dragging a tab into a group's span (between two members, or onto a member tab's center) SHALL join it to that group at the drop position via a single `groupUpdate` carrying membership AND the new strip order; dragging a member past the group's boundary SHALL remove it from the group and place it there; dragging the CHIP SHALL move the entire group as one unit (a plain `reorderTabs` block move). All existing gestures (reorder outside groups, drag-to-split, drag-onto-slot) SHALL keep working; a split pairing SHALL be unaffected by any group membership change. The content-area split/slot drop zones SHALL ignore the group-chip drag payload.
...
#### Scenario: Chip drag moves the unit

- **WHEN** the chip is dragged to another strip position
- **THEN** all members move together (order inside the group preserved), and no split/slot drop zone activates

#### Scenario: Split pairings are group-blind

- **WHEN** a split member joins or leaves a group, or a shown split member's group is collapsed
- **THEN** membership changes never alter the pairing; collapsing routes to nearest-outside per the guard, which by display-scoping simply hides the split (the same rule as any non-member activation)
